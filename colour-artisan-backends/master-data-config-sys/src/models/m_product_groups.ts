...
export class ProductGroupsSchema extends TableRecordsSchema{
    ...
    ////BUSINESS LOGICS
    getByDBVersion(db_version_id: any): Promise<ProductGroup[]>{
        return new Promise((resolve, reject) => {
            DB.raw(`select a.*, (select COUNT(k.*) 
                from "${TABLE_NAMES.ProductShadeCodes}" as k where a.id = k.product_group_id) AS products 
    ...
    getByDBVersionLite(db_version_id: any): Promise<ProductGroup[]>{
        return new Promise((resolve, reject) => {
            const table = DB<any>(this.tableName);
            table.select('*').where('db_version_id', db_version_id).where('deleted_at', null).then((val) => {
                resolve(val);
