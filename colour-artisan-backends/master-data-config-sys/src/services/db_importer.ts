...
export class DBImporter{
    ...
    static async computeSheet_1(workbook: Workbook, db_version_id: any, canSizes: any[], 
        ...
        for (let i = 0; i < pg_ids.length; i++) {
            const item = pg_ids[i];
            product_groups[i]._id =  product_groups[i].id;
        ...
        for (let i = 0; i < pb_ids.length; i++) {
            const item = pb_ids[i];
            product_bases[i]._id = product_bases[i].id;
        ...
        for (let i = 0; i < sp_ids.length; i++) {
            const item = sp_ids[i];
            sub_products[i]._id = sub_products[i].id;
    ...
    static async computeSheet_4(workbook: Workbook, db_version_id: any, canSizesSCH: CanSizesSchema, can_units: CanUnit[]) {
        ...
        sheet?.eachRow((row, rowNum) => {
            if (rowNum !== 1) {
                ...
                const cu = can_units.find((value) => value.name == cuid);
                const data: CanSize = {
                    db_version_id: db_version_id,
    ...
    static async computeSheet_5(workbook: Workbook, db_version_id: any, products: Product[], canSizes: any[]) {
    ...
    static async computeSheet_6(workbook: Workbook, db_version_id: any) {
        ...
        sheet?.eachRow((row, rowNum) => {
            if(rowNum !== 1){
                const data: ProductBasePricing = {
                    id: null,
                    db_version_id: db_version_id,
                    product_base_id: row.getCell(2).value ? row.getCell(2).value?.toString() : null,
                    can_size_id: row.getCell(3).value ? row.getCell(3).value?.toString() : null,
    ...
    static async computeSheet_7(workbook: Workbook, db_version_id: any) {
        ...
        sheet?.eachRow((row, rowNum) => {
            if(rowNum !== 1){
                const data: GeneralPricing = {
                    id: null,
                    db_version_id: db_version_id,
                    can_size_id: row.getCell(1).value ? row.getCell(1).value?.toString() : null,
